...
export const loadPins = createAsyncThunk<PinResponse, void, { rejectValue: string }>(
	'channels/loadPins',
...
export const loadChannels = createAsyncThunk<Channel[], void>('channels/loadChannels', async () => {
...
const channelsSlice = createSlice({
	...
	reducers: {
		...
		unpinChannel(state, action: PayloadAction<string>) {
			...
			state.pins.allChannels = state.pins.allChannels.filter((id) => id !== channel);
		},
	},
		...
		builder
			...
			.addCase(loadPins.fulfilled, (state, action) => {
				...
				for (const pin of action.payload.pin_channel) {
					...
					const paymentOutput = find(pin.out, (o) => o.e?.a === pinPaymentAddress);
					...
					if (paymentAmount > 0) {
						...
						const expireTime = expireMinutesFromTimestamp * msPerMinute + (pin.timestamp || 0);
