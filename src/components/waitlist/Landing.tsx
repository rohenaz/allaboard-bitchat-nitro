...
export function Landing() {
	const { loading, user, approved, waitlistMode, onWaitlist, position, total } = useAccess();
	...
	if (user) {
			...
			return (
				<Shell>
					...
					<p className="text-balance text-muted-foreground">
						...
						We'll email <span className="text-foreground">{user.email}</span> when it's your turn.
					</p>
					<SignOut />
				</Shell>
			);
		...
		return (
			<Shell>
				...
				<p className="text-balance text-muted-foreground">
					Signed in as <span className="text-foreground">{user.email}</span>. Access is invite-only
					right now — save your spot.
				</p>
				<button
					type="button"
					disabled={busy}
