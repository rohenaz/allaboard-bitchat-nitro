...
async function requestAIPSignature(hexArray: string[]): Promise<number[][]> {
	...
	return new Promise((resolve, reject) => {
		...
		const timeout = setTimeout(() => {
			...
			console.error(
				'[sendTransaction] AIP signing timeout - no response from Sigma iframe after 30s',
			);
...
export async function sendTransaction(
	...
	console.log('[sendTransaction] Starting transaction flow', {
		dataCount: dataPayload.length,
		broadcast,
	});
	...
	console.log(
		'[sendTransaction] STEP 1 - Template BEFORE signing (OP_RETURN placeholder):',
		templateHex,
	);
	...
	console.log(
		'[sendTransaction] STEP 2 - AFTER AIP signing (with user identity signature):',
		signedTemplateHex,
	);

	// Step 4: Fund and broadcast via nitro-api (proxies to Droplit with platform auth)
	console.log(
		'[sendTransaction] Step 4: Funding via nitro-api /droplit/fund (broadcast:',
		broadcast,
		')',
	);
	...
	console.log(
		'[sendTransaction] STEP 3 - AFTER Droplit funding (inputs filled, ready to broadcast):',
		fundData.rawtx,
	);
