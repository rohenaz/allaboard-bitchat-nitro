...
export const getFriends = async (idKey: string) => {
	...
	const queryFriendsB64 = (idKey: string) => btoa(JSON.stringify(queryFriends(idKey)));
...
export const getMessages = async (channelName: string): Promise<MessageResponse> => {
...
export async function getDiscordReactions(messageId: string): Promise<Reaction[]> {
