...
const BitcoinProvider: React.FC<BitcoinProviderProps> = ({ children }) => {
	...
	const [postStatus, setPostStatus] = useState<FetchStatusType>(FetchStatus.Idle);
	const [pendingFiles, setPendingFiles] = useState<PendingFile[]>([]);
	const [pendingFilesOutputs, setPendingFilesOutputs] = useState<string[][]>([]);
	const [signStatus, setSignStatus] = useState<FetchStatusType>(FetchStatus.Idle);
	const [likeStatus, setLikeStatus] = useState<FetchStatusType>(FetchStatus.Idle);
	const [friendRequestStatus, setFriendRequestStatus] = useState<FetchStatusType>(FetchStatus.Idle);
	...
	const sendPin = useCallback(
		async (pm: string, channel: string, units: number) => {
			...
			try {
				...
				const hexArray = dataPayload.map((d) => Buffer.from(d, 'utf8').toString('hex'));
				if (decIdentity) {
				...
				if (authToken) {
					const hexArray = dataPayload.map((str) => bops.to(bops.from(str, 'utf8'), 'hex'));
					// .join(" ")
	...
	const handleMessage = useCallback(
		async (pm: string, content: string, channel: string, userId?: string) => {
			...
			try {
				...
				const bPayload = [...contentPayload, ...contentFilesSeparator, ...filesPayload];
	...
	const sendFriendRequest = useCallback(
		async (friendIdKey: string, xprv: string) => {
			...
			try {
				...
				const hexArray = dataPayload.map((str) => Buffer.from(str, 'utf8').toString('hex'));
