...
test.describe('OAuth Flow with Sigma Auth', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForFunction(
      () => {
        ...
        return body?.textContent && !body.textContent.includes('Loading...');
      },
      { timeout: 10000 },
    );
  });
  ...
  test('should complete full OAuth flow - happy path', async ({ page }) => {
    ...
    await page.waitForSelector('text=Choose your login method', {
      timeout: 10000,
    });

    // 1. Click "Sign in with Bitcoin" button
    const sigmaLoginButton = page.getByRole('button', {
      name: /sign in with bitcoin/i,
    });
    ...
    await expect(
      page.getByText(/sign in with your bitcoin identity/i),
    ).toBeVisible();
  ...
  test('should handle OAuth errors gracefully', async ({ page }) => {
    ...
    await page.goto(
      '/auth/sigma/callback?error=access_denied&error_description=User+cancelled+authentication',
    );

    // 2. Should show error message
    await expect(
      page.getByText(/authentication error: access_denied/i),
    ).toBeVisible();
    await expect(
      page.getByText(/user cancelled authentication/i),
    ).toBeVisible();
